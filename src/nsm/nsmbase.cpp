...
#include "c_macros.h"                   /* not_nullptr(), warnprint(), etc. */
#include "nsm/nsmbase.hpp"              /* nsm66::nsmbase class             */
#include "nsm/nsmmessagesex.hpp"        /* nsm66::nsm new message functions */
#include "util/msgfunctions.hpp"        /* util::session_message from Cfg66 */
...
/*--------------------------------------------------------------------------
 * OSC nsmbase handler static functions
 *--------------------------------------------------------------------------*/

/**
 *  A handler for the message: osc::tag::reply =  { "/reply" + "ss" }
 *
 *  See nsmbase::nsm_reply() for more information.
 */

int
nsmbase::osc_nsm_reply
(
    const char * path,
    const char * types,
    lo_arg ** /* argv */,
...
/**
 *  A handler for the message: osc::tag::error =  { "/error" + "sis" }
 *
 ...
 *
 *  See nsmbase::nsm_error() for more information.
 */

int
nsmbase::osc_nsm_error
(
    const char * path,
    const char * types,
    lo_arg ** argv,
...
)
{
    ...
    int rc = osc::lowrapper::osc_error
    (
        path, types, argv, argc, msg, user_data
    );
    if (rc == osc::osc_msg_handled())
        pnsmc->nsm_error(int(argv[1]->i), &argv[2]->s);
...
{
    if (not_nullptr(m_server_thread))
    {
        int rcode = lo_server_thread_start(m_server_thread);
        if (rcode == 0)                                     /* successful?  */
            util::session_message("OSC server thread started");
        else
            util::error_message("OSC server thread start failed");
    }
}

/**
 *  TODO: Investigate and reconcile.
 */
 ...
 *
 *  Compare to osc::lowrapper::init().
 */

bool
nsmbase::initialize ()
{
    lo_address laddr = lo_address_new_from_url(CSTR(nsm_url()));
    bool result = not_nullptr(laddr);
    if (result)
    {
        const int proto = lo_address_get_protocol(laddr);
        ...
        ps += " OSC protocol";
        util::session_message(ps);
        m_server_thread = lo_server_thread_new_with_proto(NULL, proto, NULL);
        result = not_nullptr(m_server_thread);
        if (result)
        {
            m_lo_server = lo_server_thread_get_server(m_server_thread);
            result = not_nullptr(m_lo_server);
            if (result)
            {
                /*
                 * Similar in the base class, though.
                 */

                add_thread_method(osc::tag::error, &nsmbase::osc_nsm_error);
                add_thread_method(osc::tag::reply, &nsmbase::osc_nsm_reply);
                ...
                util::error_message("NSM bad server");
        }
        else
            util::error_message("NSM bad server thread");
    }
    else
        util::error_message("NSM bad server address");
...
{
    bool result = not_nullptr_2(address(), server());
    if (! result)
        util::error_message("Null OSC address or server");
...
nsmbase::msg_check (int timeoutms)
{
    ...
    if (timeoutms > 0)
    {
        ...
        {
            result = true;
            util::session_message("NSM waiting for reply...");
        ...
        if (! result)
            util::error_message("NSM no reply!");
    }
    return result;
}
 ...
 *
 *      -   osc::tag::cliclean:     "/nsm/client/is_clean"
 *      -   osc::tag::clidirty:     "/nsm/client/is_dirty"
 *
 *  It sets the m_dirty flag accordingly.
 *
 *  The old code is not robust enough, we need to get the pattern ("")\
 *  as well.
 *
 *      const char * path = CSTR(nsm::get_dirtiness_msg(isdirty));
 */
...
nsmbase::progress (float percent)
{
    ...
    if (result)
    {
        std::string message;
        std::string pattern;
        bool ok = nsm::client_msg(osc::tag::cliprogress, message, pattern);
        if (ok)
        {
            (void) lo_send_from             /* "/nsm/client/progress" "f"   */
            (
                address(), server(), LO_TT_IMMEDIATE_2,
                CSTR(message), OPTR(pattern), percent
            );
...
nsmbase::is_dirty ()
{
    ...
    if (result)
    {
        std::string message;
        std::string pattern;
        bool ok = nsm::client_msg(osc::tag::clidirty, message, pattern);
        if (ok)
        {
...
nsmbase::is_clean ()
{
    ...
    if (result)
    {
        std::string message;
        std::string pattern;
        result = nsm::client_msg(osc::tag::cliclean, message, pattern);
        if (result)
...
nsmbase::message (int priority, const std::string & mesg)
{
    ...
    if (result)
    {
        std::string message;
        std::string pattern;
        bool ok = nsm::client_msg(osc::tag::climessage, message, pattern);
        if (ok)
        {
            lo_send_from                    /* "/nsm/client/message" "is"   */
            (
                address(), server(), LO_TT_IMMEDIATE_2,
                CSTR(message), OPTR(pattern),
                priority, CSTR(mesg)
            );
...
{
    return send_nsm_reply("/nsm/client/open", errorcode, msg);  /* cliopen  */
}

bool
nsmbase::save_reply (nsm::error errorcode, const std::string & msg)
{
    return send_nsm_reply("/nsm/client/save", errorcode, msg);  /* clisave  */
}
...
)
{
    ...
    if (result)
    {
        ...
        {
            if (client_msg(osc::tag::reply, message, pattern))
        ...
        {
            if (client_msg(osc::tag::error, message, pattern))
            {
                rc = lo_send_from
                (
                    address(), server(), LO_TT_IMMEDIATE_2,
                    CSTR(message), OPTR(pattern), OPTR(path),
                    static_cast<int>(errorcode), CSTR(replymsg)
                );
            }
            replytype = "error";
        }
...
(
    const std::string & appname,        /* actually a package name, "nsm66" */
...
)
{
    ...
    bool result = lo_is_valid();
    if (result)
        result = nsm::server_msg(osc::tag::guiannounce, message, pattern);

    if (result)
    {
        const char * packagename = CSTR(appname);
        const char * app = CSTR(exename);
        const char * caps = CSTR(capabilities);
        int pid = int(getpid());
        int rc = lo_send_from           /* "/nsm/server/announce" "sssiii"  */
        (
            address(), server(), LO_TT_IMMEDIATE_2,
            CSTR(message), OPTR(pattern), packagename, caps, app,
 ...
 * But see nsmcontroller::osc_handler()'s "/reply" clause.
 * It can show a server-announce, a session name, a ping response,
 * and a generic reply.
 */
...
{
    nsm::incoming_msg("Server reply", message, pattern);
}

/*
 *  Handles the base server error:
 *
 *      /error i:errcode s:errormessage (type-specification "sis").
 *
 *  At this point, osc::lowrapper::osc_error() has already been
 *  called. It handles announce errors and emits a "client error" message.
 */

void
nsmbase::nsm_error (int errcode, const std::string & errmesg)
{
    /*
     * is_active(false);
     */
...
)
{
    ...
    if (client_msg(t, message, pattern))
    {
        ...
        else
        {
            (void) lo_server_thread_add_method
            (
                m_server_thread, CSTR(message), OPTR(pattern), f,
                userdata  // this
            );
...
{
    int rcode = send_from(CSTR(message), OPTR(pattern));
    bool result = rcode != (-1);
    if (result)
...
bool
nsmbase::send_from_client (osc::tag t)
{
    std::string message;
    std::string pattern;
...
get_session_url (const std::string & env_value)
{
    ...
    char * url = secure_getenv(CSTR(env_value));
#else
    char * url = std::getenv(CSTR(env_value));
#endif
